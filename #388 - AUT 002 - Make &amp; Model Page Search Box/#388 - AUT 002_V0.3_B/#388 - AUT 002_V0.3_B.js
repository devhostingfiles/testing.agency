...
$.fn.addVariantStyle = function(){
    var style = '' +
        ...
        '.ta-388 #buy .buy_search .ser_row_1 .block_1 { position: relative; }' +
        ...
        '.ta-388 #buy .buy_search.p_bot .post-locate { width: 47px; height: 41px; background: url(/Content/images/post-detact.png) no-repeat; position: absolute; right: auto; margin-left: -48px; top: 2px; }' +
        ...
        '.ta-388 .ta-model-page { width: 100% !important; text-align: center; position: relative; }' +
        '.ta-388 .ta-model-page + .block_1 { display: none; }' +
        '.ta-388 .ta-model-page #txtPostCode { width: 50% !important; display: inline-block; }' +
        '.ta-388 #btbNumberOfCars { margin-bottom: 10px; margin-top: 25px; text-align: center; display: block; font-size: 45px; color: #191366; font-family: \'Roboto\',sans-serif; }' +
        '@media (max-width: 767px) {' +
        '.ta-388 .ta-model-page-block { padding-left: 0 !important; padding-right: 0 !important; }' +
        '.ta-388 .ta-model-page #txtPostCode { width: 100% !important; }' +
        '.ta-388 #btbNumberOfCars { font-size: 35px !important; margin-bottom: 10px; margin-top: 10px; }' +
        '}' +
        '@media (max-width: 480px) {' +
        '.ta-388 #btbNumberOfCars { font-size: 30px !important; line-height: 36px !importantx; line-height: 36px; color: #191366; font-family: \'Roboto\',sans-serif;}' +
        '}' +
        '';
...
$.fn.addFieldsColumns = function(){
    ...
    $(".block_1:last-child", this).prepend($(".block_1:first-child .fancy-select", this).detach());
    $(".block_1:last-child .fancy-select", this).first().wrap($("<div>", { "class" : "block_2 ta-col-makename ta-col-left" }));
    $(".block_1:last-child .fancy-select", this).last().wrap($("<div>", { "class" : "block_2 ta-col-modelname ta-col-right" }));
    $(".ta_row_1 > .block_1:first-child").html( $('.ta_row_button .block_1').first().find("input"));
    ...
    if ( $("#modelname").val() !== "" && $("#modelname").val() !== "select-model-any" && $("#modelname").val() !== null ) {
        $(".buy_search.p_bot > .container > .ta_row_1").addClass("ta-model-page-block");
        $(".ta-model-page-block > .block_1").first().addClass("ta-model-page");
    } else {
        GetModelData();
    }
...
$.fn.addNumberOfCars = function() {
    if ($("#btbNumberOfCars").length === 0) {
        ...
        makename = $("#makename").val() !== "" && $("#makename").val() !== "select-make-any" ? makename : "";
        modelname = $("#modelname").val() !== "" && $("#modelname").val() !== "select-model-any"  ? modelname : "";
        ...
        $(".ta-make").text(makename);
        $(".ta-model").text(modelname);
        getNumberOfCars();
    }
...
$(document).ready(function(){
    ...
    if ( $("select#modelname").length !== 0 ) {
        ...
        $('body').addBodyClasses();
        $('body .buy_search.p_bot > .container > .ser_row_1:first-child').addFieldsColumns();
